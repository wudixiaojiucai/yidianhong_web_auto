...
class TestContacts:
    ...
    def test_add_user(self, index_fixture: IndexPage, del_fixture, user_normal_data):
        """
        添加成员 - 信息正确输入
        """
        page = index_fixture
        page.to_user_list_page().to_add_user_page().add_user_normal(
    ...
    def test_add_user_fail(self, index_fixture: IndexPage, user_fail_data):
        """
        添加成员 - 输入已存在的账号
        添加成员 - 输入已存在的手机号
        """
        page = index_fixture
        res = page.to_user_list_page().to_add_user_page().add_user_fail(
